...
pub struct Analysis { request: RequestParameter }
...
impl Analysis {
    ...
    fn save_report_to_db(&self, code: &str, name: &str, report: &AnalysisReport, report_json: &serde_json::Value) {
    ...
    fn query_report_list(&self, code: &str, limit: i64) -> Result<Vec<Value>> {
        let mut q = self.select()
            .columns(cols![
                "id", "stock_code", "stock_name", "sentiment_score",
                "decision_type", "operation_advice", "analysis_summary", "create_time"
            ])
    ...
    fn query_history_list(&self, code: &str, limit: i64, offset: i64) -> Result<Vec<Value>> {
        let mut q = self.select()
            .columns(cols![
                "id", "stock_code", "stock_name", "sentiment_score",
                "decision_type", "operation_advice", "analysis_summary",
                "risk_warning", "report_type", "query_id", "status",
    ...
    fn query_history_detail(&self, id: i64) -> Result<Option<Value>> {
        let res = self.select()
            .columns(cols![
                "id", "stock_code", "stock_name", "sentiment_score",
                "decision_type", "operation_advice", "analysis_summary",
                "risk_warning", "report_json", "context_snapshot",
                "report_type", "query_id", "status", "create_time", "modify_time"
            ])
            .r#where(conds![{ "id" = id }])
            .one()?;
        Ok(if res.is_null() { None } else { Some(res) })
    ...
    fn query_history_search(&self, keyword: &str, limit: i64) -> Result<Vec<Value>> {
        ...
        let sql = format!("SELECT id, stock_code, stock_name, sentiment_score, decision_type, operation_advice, \
