...
/// 决策信号提取服务
pub struct DecisionExtractorService;

impl DecisionExtractorService {
    /// 创建决策信号提取服务实例
    pub fn new() -> Self {
        Self
    }

    /// 请求分发 - 可用方法: extract, extract_batch, data_quality
    pub async fn dispatch(&self, method: &str, params: &Value) -> DsaResult<Value> {
        match method {
            "extract" => self.extract(params).await,
            "extract_batch" => self.extract_batch(params).await,
    ...
    /// 从分析报告中提取决策信号
    async fn extract(&self, params: &Value) -> DsaResult<Value> {
        let analysis_id = utils::param_i64(params, "analysisId");
        if analysis_id == 0 {
            return Err(DsaError::Validation("请提供analysisId".to_string()));
        }
        ...
        if rows.is_empty() {
            return Err(DsaError::Validation(format!("分析记录不存在: {}", analysis_id)));
        }
        ...
        // 从 marketContext 推断 market phase
    ...
    /// 批量提取: 从所有尚无信号的分析中提取
    async fn extract_batch(&self, params: &Value) -> DsaResult<Value> {
        ...
        .map_err(|e| DsaError::Database(format!("查询孤儿分析失败: {}", e)))?;
        ...
        for row in &rows {
            ...
            match self.extract(&extract_params).await {
                Ok(_) => {
                    extracted_count += 1;
                }
                Err(e) => {
    ...
    async fn data_quality(&self, params: &Value) -> DsaResult<Value> {
        ...
        .map_err(|e| DsaError::Database(format!("查询K线覆盖度失败: {}", e)))?;
        ...
        .map_err(|e| DsaError::Database(format!("查询新闻覆盖度失败: {}", e)))?;
        ...
        .map_err(|e| DsaError::Database(format!("查询分析覆盖度失败: {}", e)))?;
        ...
        .map_err(|e| DsaError::Database(format!("查询信号覆盖度失败: {}", e)))?;
